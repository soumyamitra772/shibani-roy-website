...
import { dbService, DEFAULT_TERMS_CONTENT } from '../services/db';
...
export default function TermsPage() {
  const [data, setData] = useState<LegalPageData>(DEFAULT_TERMS_CONTENT);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    let isMounted = true;
    dbService.getLegalPage('terms').then((page) => {
      if (isMounted && page) {
        setData(page);
      }
    }).catch((err) => {
      console.warn('Error fetching terms page:', err);
    }).finally(() => {
